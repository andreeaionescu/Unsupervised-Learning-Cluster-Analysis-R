...
#table - tabelul prelucrat
#k - vector cu variabilele nenumerice
#tableT - tabelul test
...
#Calcul distante
	...
	clust = hclust(d(table))
	labels=rownames(table)
	plot(clust,labels=rownames(table),hang=-1)

#Evolutia clasificarii
	index=clust$merge
	m=nrow(index)
	H=character(m)
	for(i in 1:m){
